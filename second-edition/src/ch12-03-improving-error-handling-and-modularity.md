...
тестирование функции `main`, весь код, который необходимо протестировать, надо перенести
...
решения поставленной задачи. Код функционала анализа переменных командной
...
из консоли в переменные (в кортеж) и её вызов в функции `main`</span>
...
В тоже время, у неё есть ясные плюсы, такие как отсутствие необходимости следить за
...
вызов макроса `panic!`, но отличается от него возвращением значения кода ошибки.
...
функцией `run`. Т.к. `run` не возвращает информация для анализа - это может быть только
информация об ошибке, то её мы и будем анализировать.
...
строку `use minigrep::Config` в область видимости и префикс к функции `run`. Код
программы 12-14:
