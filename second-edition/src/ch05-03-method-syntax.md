...
> аналогичны.
>
> Rust не имет такого эквивалента (оператора `->`), наоборот, в Rust имеется функционал,
...
> добавляет `&`, `&mut`, или `*` так, чтобы `object` имел соответсующие опции.
> Другими словами, строки *p1.distance(&p2);* и *(&p1).distance(&p2);* эквивалентны:
>
>```rust
...
(читать значение (`&self`), изменять содержимое (`&mut self`) или сохранять значение (`self`)).
...
`self` и все эти параметры работают также, как и параметры функции.
...
Такие функции часто используются для инициализации экземпляра структуры.
Пример:
