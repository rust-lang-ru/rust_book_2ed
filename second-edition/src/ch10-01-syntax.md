...
<span class="caption">Код программы 10-4: Две функции, отличия которых (кроме названий) только
...
как мы даём имя параметру функции. Назовём этот тип `T`. В качестве этого имени может
...
Читается это определение следующим образом: обобщенная функция `largest` имеет тип параметров
...
<span class="caption">Код программы 10-5: Объявление функции `largest`, которая использует
...
<span class="caption">Код программы 10-6: Использование структуры `Point` содержащей поля
`x` и `y` типа `T`</span>
...
Если структура будет инициализирована различными числовыми типами данных - код не скомпилируется:
...
<span class="caption">Код программы 10-7:Пример ошибки. Поля `x` и `y` должны быть инициализированы
...
<span class="caption">Код программы 10-8: Структура `Point` имеет два поля разного типа</span>
...
Также как и в структурах, перечисления могут иметь обобщенные типы данных. Мы уже использовали
...
Обратите внимание, что параметры типов `T` и `U` объявляются в блоке реализации после
...
Вам может быть интересно какие накладные расходы могут быть у программ, использующих дженерики. Хорошие новости, что Rust не замедляет ваш код, использующий дженерики, выводя конкретный тип на этапе компиляции. Данный процесс называется *мономорфизация* -- конвертирование обобщенного кода в код с указанием конкретных типов данных.
...
При компиляции произойдет *мономорфизация*. Компилятор прочитает значения
внутри значений перечисления `Option` и создаст необходимые типы `Option<i32>` и `Option<f64>`.
...
Подведём итоги.
Мы познакомились с обобщенным программированием в Rust. Благодаря внутренней оптимизации, имеющей имя *мономорфизация*,
обобщенного кода, нет накладных расходов при работе программ. Процесс *мономорфизации* делает использование дженериков в Rust очень эффективным.
