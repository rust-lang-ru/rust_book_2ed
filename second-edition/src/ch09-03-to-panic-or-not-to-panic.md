...
значение `Result`? Если код уже находится в режиме паники - нет возможности восстановления.
...
ненужно. Когда вы возвращаете значение перечисления `Result` пользователю
...
Далее мы разберем работу с макросом `panic!`. Мы напишем и разберем код, протестируем
...
программа может войти в ошибочное состояние. Ошибочное состояние - это когда не выполняются
предположения, соглашения, когда могут быть недоступны или утеряны какие-либо данные.
Ошибочное состояние - это непредвиденная основной логикой программы поведение.
...
реакции на них может быть макрос `panic!` и информирование об этом. Также макрос `panic!`
...
Когда код программы сначала проверяет корректность данных, можно вызвать макрос `panic!` и проинформировать о том, что входные данные неверны.
...
В стандартной библиотеке наиболее частый вариант обработки ошибок - использование
`panic!`. Это делается для безопасности. После получения данный информации, программист
решает что делать и как лучше в данном конкретном случае исправить ошибку.
...
принятия решений. Чем больше будет ограничений на тип данных, тем безопаснее будет
ваш код.
...
<span class="caption">Код программы 9-8: Тип `Guess`, который создаёт свой экземпляр только
на основе данных, соответствующих заданным условиям</span>
...
находится между 1 и 100. Если значение выходит за эти пределы, будет вызван
...
других параметров. Этот метод возвращает значение поля `value` типа `u32`.
Эта синтаксическая структура похожа на реализацию свойства для чтения. Этот метод
необходим для доступа к значению поля `value`, т.к. доступ к этому полю закрыт. 
Пользователи данной структуры обязаны использовать
...
сигнализирует о том, что программа находится в аварийной ситуации и завершает рабооту
программы вместо того, чтобы использовать некорректные данные.
Тип `Result` позволяет программисту проанализовать случившуюся проблему и выбрать
самостоятельно путь дальнейшей работы.

Вы уже видели, что стандартная библиотека часто использует шаблонные типы данных (дженерики)
,например, в `Option` и `Result`. Далее, мы поподробнее поговорим о дженериках и их использовании
в Вашем коде.
