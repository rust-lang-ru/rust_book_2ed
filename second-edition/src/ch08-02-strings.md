...
После этого `s` будет содеражть “lol”.
...
Например, посмотрим этот ошибочный код:
...
Ещё одна причина по которой Rust не позволяет получать символ по индексу,
заключается в том, что ожидается постоянная сложность данной операции (O(1)). 
Но Rust не может это обеспечить потому, что ему нужно пройти через всю строку,
что бы корректно определять начало и конец символа.
