...
fn calculate_length(s: &String) -> usize { // s это ссылка на String
    s.len()
} // s выходит из области видимости. Но поскольку он не собственник
  // ссылаемого объекта, ничего не происходит.
```
...
Как и переменные, ссылочные переменные неизменяемые. Т.е. мы не можем менять то, на что ссылаемся.

### Изменяемые ссылочные данные

Чтобы исправить ошибку в предыдущем примере 4-9, надо сделать небольшие
изменения в коде:
...
Первое, мы сделали переменную `s` изменяемой при помощи `mut`. Затем мы создали изменяемую ссылку `&mut s` и признали её изменяемой ссылкой в функции change: `some_string: &mut string`.

Но изменяемая ссылка имеет одно большое ограничение: Вы можете иметь одну, и только одну изменяемую ссылку на объект в конкретной области видимости. 
...
Обратите внимание, что вы не можете иметь изменяемую ссылочную переменнную, пока
существует неизменяемая ссылочная переменная. Наличие множества неизменяемых переменных допускается,
т.к. они не могут изменить данные значения переменной. Но пользователи неизменяемой ссылки не ожидают, что значение 
переменной изменится ссылкой, которая изменит значение переменной.
...
компилятор Rust гарантирует защиту от создания подобных ссылок.
...
1. В одной области видимости одновременно может существовать только один тип ссылочных переменных на одни данные, но не оба вместе:
  - одна единственная изменяемая ссылка (по-другому это называется "эксклюзивная ссылка на изменяемые данные"),
  - любое количество неизменяемых ссылок (соответственно "разделяемые ссылки на неизменяемые данные").
