...
необходимо превратить её в массив байтов, используя метод `as_bytes`:
...
fn main() {
    ...
    let word = first_word(&s); // word имеет значение 5

    s.clear(); // clear() очищает String, делая её равной "".

    // word по прежнему имеет значение 5, но доступа к строке
    // по этому значению уже нет!
}
```
...
Внутренне, переменная типа срез устроена следующим образом:
...
Вооружившись знаниями о срезах по-новому можно посмотреть на
...
Тип `s` является `&str` - это срез бинарных данных специального вида.
...
Если мы используем срез, мы может его передавать в методы.
...
fn main() {
    ...
    // first_word принимает срез String'а
    let word = first_word(&my_string[..]);
    println!("{}",word);
    let my_string_literal = "hello world";

    // first_word works принимает срез строкового литерала
    let word = first_word(&my_string_literal[..]);
    println!("{}",word);
    // строковые литералы уже являются срезами,
    // поэтому это работает тоже, без синтаксиса срезов!
    let word = first_word(my_string_literal);
    println!("{}",word);
}
```
