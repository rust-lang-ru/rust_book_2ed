...
Это системы сборки мусора (Garbage Collector), ручное управление выделенными ресурсами. Rust предлагает
...
для создания безопасных и эффективных программы.

Понимание концепции владениия даст вам основу для понимания всех остальных особенностей
Rust. В этой главе вы будете изучать владение на примере работы с строковым типом
данных.
...
> Работа со стеком даёт большой выигрыш по времени, т.к. нет необходимости системе в
> поиске нужных данных, а также все сохраняемые там данные имеют известный размер.
>
...
> но всё же понимание как эти системы распределения памяти работают поможет в более
> глубоком понимании теоретического материала.
>
...
когда их значение и размер заранее не известны (например, при вводе данных с клавиатуры),
...
Двойное двоеточие (`::`) - это оператор, который определяет пространство имён типа.
...
fn main() {
    ...
    s.push_str(", world!"); // push_str() добавляет строку к другой строке

    println!("{}", s); // Это напечатает `hello, world!`
}
...
технологии), где используется сборщик мусора. Там где нет сборщика мусора,
...
```rust,ignore
{
    let s = String::from("hello"); // s существует с данной строчки

    // действия с переменной s
}                                  // s вышла из своей области видимости и
                                   // больше недоступна
```
...
Это шаблон сильно влияет на стиль и способ создания программ на Rust. Поведение программы
...
<span class="caption">Figure 4-3: Представление переменной `s1` в памяти, имеющей
 ...
 Длина (в байтах) содержит величину данных, которые в данный момент используются.
 ...
 Это представление отличается от того, когда бы копировалось всё содержимое кучи.
...
4-4 показывает, что имеются две ссылки на одни данные. Это является проблемой и
источником потенциальной ошибки, т.к. переменные `s2` и `s1` обе вышли за границы
области видимости и ссылаются на одни и те же данные. Эта ошибка называется
...
В терминальной строке вы увидите подробное сообщение, информирующее о том, что переменную
...
просто нет. Только одна переменная действительно имеет права на данные - это переменная
`s2`.
...
* Логический тип данных `bool` (значения `true` и `false`)
...
fn main() {
    ...
    takes_ownership(s);             // значение s перемещено в функцию
    //println!("{}", s);    // ... и больше недоступно здесь
    let x = 5;                      // x входит в область видимости

    makes_copy(x);                  // значение x перемещено в функцию
    println!("{}", x);              // но i32 реализует Copy, так что
                                    // может быть использовано далее

} // здесь x и s выходят из области видимости. Но s было перемещено,
  // поэтому ничего плохого здесь не случится

fn takes_ownership(some_string: String) { // some_string входит в область видимости
    println!("{}", some_string);
} // Здесь some_string выходит из области видимости. 
  // Для него вызывается drop 

fn makes_copy(some_integer: i32) { // some_integer входит в область видимости
    println!("{}", some_integer);
} // Здесь some_integer выходит из области видимости, но ничего не случится
```
...
fn main() {
    ...
    let s3 = takes_and_gives_back(s2);  // s2 перемещена в функцию
    //println!("{}",s2);   // takes_and_gives_back переместил
    println!("{}",s3);     // возвращаемое значение в s3
} // Здесь s3 выходит из области видимости. Для него вызывается drop
  // s2 выходит из области видимости, но ничего не происходит, т.к.
  // значение было перемещено. s1 выходит из области видимости и для
  // него вызывается drop
...
fn gives_ownership() -> String {     // gives_ownership перемещает
    ...
    println!("{}",some_string);
    some_string                              // some_string возвращается
                                             // и перемещается из функции
}

// takes_and_gives_back забирает строку во владение и возвращает её
fn takes_and_gives_back(a_string: String) -> String { // a_string входит
                                                      // в область видимости

    a_string  // a_string возвращается и перемещается из функции
}
```
...
fn calculate_length(s: String) -> (String, usize) {
    let length = s.len(); // len() возвращаети длину строки String
...
доступные программисту при работе с памятью. К счастью, существуют
