...
Строка `use a::series::of;`, что в данной области видимости могут использоваться элементы,
которые находятся в модуле `of`. Их можно вызывать просто указывая префикс имени этого
модуля `of::`.

В область видимости попадают только элементы модуля. Подчиненные модули не включаются.
...
Т.к. перечисления можно назвать разновидностью пространств, то можно указать
...
Как же вызвать функцию `client::connect` из модуля `tests`? В модуле `tests` мы можем использовать `super` для того чтобы переместиться по модульной иерархии
...
Теперь когда Вы выполните команду `cargo test`, Вы увидите следующий вывод:
