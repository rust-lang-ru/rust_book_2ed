...
> командной строке, то используйте свою любимую интеграционную среду разработки.
...
Эта строка объявляет *функцию* в Rust. Функция `main` - особенная: первый код, который
запускается для каждой исполняемой программы Rust. Первая строка объявляет функцию
с именем `main`, которая не имеет параметров и ничего не возвращает.
Если бы были параметры, их имена были бы заключены в круглые скобки `(` и `)`.
...
Есть определённые стилистические уточнения. Первое, в стиле написания кода
...
который поможет писать программы Rust.
...
Мы передали аргумент `--bin` в `cargo new`, потому что наша цель сделать исполняемое
приложение, а не библиотеку. `hello_cargo` - это название нашего нового Rust проекта.
...
Отлично! Если всё в порядке, в терминале напечатается "Hello, world!".
