...
Существует еще одно сходство между перечислениями и структурами: для обоих можно
определить методы с помощью `impl`. Вот метод с именем `call`, который мы 
можем определить в нашем перечислении `Message`:
...
одного из своих элементов. [Вот как он определяется в стандартной библиотеке][option]<!-- ignore -->:
...
использовать в коде `Some` и `None` без префикса `Option::`.
