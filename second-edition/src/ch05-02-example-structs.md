...
Мы использовали вывод `Debug`, который позволяет нам напечатать содержимое
...
Было бы лучше, если бы поведение было связано с структурой, т.к. функция может работать
только с ней. В следующей главе мы рассмотрим как улучшить код и реализовать
эту связь. Мы узнаем как сделать из обособленной функции метод структуры.
