...
Тип `if`-выражения должен быть логическим:
...
Для кода с большим количеством `else if`-выражений больше подойдёт конструкция `match`.
О ней мы расскажем в главе 6.

#### Использование `if` в `let`-операторах

По определению выражений, мы можем использовать их в операторах.
Пример, Listing 3-4:
...
Такое поведение кода делает вашу работу предсказуемой и защищает от потенциальных
ошибок.
...
Ключевое слово `loop` - это бесконечный цикл.
