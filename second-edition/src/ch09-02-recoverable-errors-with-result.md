...
`T` и `E` параметры перечисления. `T`  - это тип, который будет возвращён, при
успехе, а `E` при ошибке.

Давайте вызовем функцию, которая возвращает `Result`, т.к. в функции может
произойти ошибка. Попытаемся открыть файл:
...
<span class="caption">Код программы 9-2: Открытие файла</span>
...
<span class="caption">Код программы 9-3: Использование выражения `match` для обработки
`Result`</span>

Обратите внимание, что перечисление `Result`, также как `Option` входит в состав
экспорта по умолчанию.

Здесь значение `Ok` содержит значение типа `File`.
...
Пример создания нового файла при отсутствии запрашиваемого файла:
...
<span class="caption">Код программы 9-4: Обработка различных ошибок несколькими способами</span>
...
Есть ещё один метод похожий на `unwrap`. Используя `expect` вместо `unwrap`
можно предоставить хорошие информативные описания ошибок:
...
Мы используем `expect` таким же образом, как и `unwrap`: возвращаем ссылку на файл или
вызываем макрос `panic!`.

### Генерировании ошибок

Когда вы пишете функцию, в результате работы которой может произойти непредвиденная
ошибка, вместо того, чтобы обрабатывать эту ошибку вы можете создать её подробное
описание и передать ошибку по цепочке на верхний уровень обработки кода.
...
<span class="caption">Код программы 9-5: Эта функция, которая возвращает ошибки вызова
...
Тело этой функции начинает с вызова функции `File::open`. Далее мы получаем результат
анализа чтения файла функцией `match`. Если функция `File::open` сработала
успешно, мы сохраняем ссылку на файл в переменную `f`, и программа продолжает свою
работу.

Далее мы создаём строковую переменную `s` и вызываем метод файла `read_to_string`,
которая читает содержимое файла, как строковые данные в переменную `s`. Результатом
...
количество потенциальных ошибок. Написанный в предыдущем примере сокращенный код можно
